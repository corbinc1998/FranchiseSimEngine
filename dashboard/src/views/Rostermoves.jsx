...
function InjuryTag({ lengthType, weeksRemaining }) {
  ...
  if (lengthType === 'weeks') {
    return <span className={`chip ${weeksRemaining <= 2 ? 'chip-amber' : 'chip-red'}`}>{weeksRemaining}w</span>
  }
  return null
}
...
function PriorityTag({ priority }) {
  ...
  if (priority === 'medium') return <span className="chip chip-amber">Med</span>
...
export default function RosterMoves({ season, cfgData }) {
  ...
  useEffect(() => {
    ...
    Promise.all([api.injuries(season), api.gmLatest(season)])
      .then(([inj, gm]) => { setInjuries(inj.injuries || []); setGmData(gm) })
      .catch(console.error)
  ...
  const TABS = [
    { id: 'injuries', label: `Injuries (${injuries.length})` },
    { id: 'depth',    label: `Depth Chart (${allFlags.length})` },
  ]
  ...
  return (
    <>
      <div className="view-header">
        ...
        <div className="view-sub">Season {season}{week ? ` · Week ${week}` : ''}</div>
      </div>
                ...
                <table className="data-table">
                  <thead>
                    <tr>
                      ...
                      <th>Injury</th>
                      <th>Status</th>
                      <th className="right">Wk</th>
                      <th className="right">Return</th>
                    </tr>
                  </thead>
                  <tbody>
                    {injuries
                      .sort((a, b) => ({ career: 0, season: 1, weeks: 2 }[a.length_type] ?? 3) - ({ career: 0, season: 1, weeks: 2 }[b.length_type] ?? 3))
                      .map((inj, i) => (
                        <tr key={i}>
                          <td>
                            <TeamCell teamId={inj.team} cfgData={cfgData} logoSize={18} fontSize={12} />
                          </td>
                          ...
                          <td>
                            <InjuryTag lengthType={inj.length_type} weeksRemaining={inj.weeks_remaining} />
                          </td>
                          <td className="right" style={{ fontFamily: 'var(--mono)', color: 'var(--text-dim)' }}>
                            {inj.week_injured != null ? `W${inj.week_injured}` : '—'}
                          </td>
                          <td className="right" style={{ fontFamily: 'var(--mono)', color: 'var(--text-dim)' }}>
                            {inj.length_type === 'weeks' && inj.expected_return_week != null ? `W${inj.expected_return_week}`
            ...
            : <>
                {highFlags.length > 0 && (
                  <div style={{ marginBottom: 16 }}>
                    <div className="section-label" style={{ color: 'var(--red)' }}>High Priority — Make Change Now</div>
                    <div className="card">
                      <table className="data-table">
                        <thead>
                          <tr>
                        ...
                        <tbody>
                          {highFlags.map((f, i) => (
                            <tr key={i}>
                              <td><TeamCell teamId={f.team_id} cfgData={cfgData} logoSize={18} fontSize={12} /></td>
                              <td style={{ fontFamily: 'var(--mono)', color: 'var(--text-dim)' }}>{f.position}</td>
                              <td style={{ color: 'var(--green)', fontWeight: 500 }}>{f.recommended?.name}</td>
                              <td style={{ fontFamily: 'var(--mono)' }}>{f.recommended?.overall}</td>
                              <td style={{ color: 'var(--text-dim)' }}>{f.current_starter?.name}</td>
                              <td style={{ fontFamily: 'var(--mono)', color: 'var(--text-dim)' }}>{f.current_starter?.overall}</td>
                          ...
                          {medFlags.map((f, i) => (
                            <tr key={i}>
                              ...
                              <td style={{ fontFamily: 'var(--mono)', fontSize: 11 }}>
                                {f.recommended?.overall}{f.recommended?.age ? ` / ${f.recommended.age}` : ''}
                              </td>
                              <td style={{ color: 'var(--text-dim)' }}>{f.current_starter?.name}</td>
                              <td style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text-dim)' }}>
                                {f.current_starter?.overall}{f.current_starter?.age ? ` / ${f.current_starter.age}` : ''}
                              </td>
                              <td style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text-dim)' }}>
                                {(f.reasons || []).join(', ')}
                              </td>
